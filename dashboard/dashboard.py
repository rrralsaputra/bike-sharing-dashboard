...
# ── Warna sistem ─────────────────────────────────────────────────────────────
# BASE  : warna netral untuk bar biasa
# HIGHLIGHT : warna aksen untuk nilai tertinggi (menonjol)
# LOW   : warna redup untuk nilai terendah
BASE      = "#A8C8E8"   # biru muda — netral
HIGHLIGHT = "#1A6BB5"   # biru tua  — nilai tertinggi (fokus)
LOW       = "#D0E4F5"   # biru pucat — nilai terendah
LINE_COL  = "#1A6BB5"
 
# Warna untuk 2 kategori (casual vs registered) — konsisten di seluruh dashboard
COL_CAS = "#F4845F"   # oranye  → casual
COL_REG = "#1A6BB5"   # biru    → registered
...
with c1:
    # Line chart tren harian — tidak berubah, memang tepat untuk data waktu
    fig, ax = plt.subplots(figsize=(9, 4))
    ax.fill_between(day_f["dteday"], day_f["cnt"], alpha=0.15, color=LINE_COL)
    ax.plot(day_f["dteday"], day_f["cnt"], color=LINE_COL, linewidth=1.6)
    ax.set_xlabel("Tanggal", labelpad=6)
...
with c2:
    # PERUBAHAN: Pie → Horizontal bar chart
    # Pie chart sulit dibaca secara presisi; bar chart langsung terlihat selisihnya
    total_all = total_casual + total_reg
    pct_cas   = total_casual / total_all * 100 if total_all else 0
    pct_reg   = total_reg    / total_all * 100 if total_all else 0
 
    fig, ax = plt.subplots(figsize=(4, 2.2))
    bars = ax.barh(["Penyewa Biasa\n(Casual)", "Member\n(Registered)"],
                   [pct_cas, pct_reg],
                   color=[COL_CAS, COL_REG],
                   edgecolor="white", height=0.55)
    for bar, val in zip(bars, [pct_cas, pct_reg]):
        ax.text(bar.get_width() - 2, bar.get_y() + bar.get_height() / 2,
                f"{val:.1f}%", va="center", ha="right",
                fontsize=11, fontweight="bold", color="white")
    ax.set_xlim(0, 100)
    ax.set_xlabel("Porsi Pengguna (%)")
    ax.set_title("Komposisi Pengguna", fontsize=11, fontweight="bold")
...
def bar_colors_highlight(values):
    """Semua bar warna BASE; bar tertinggi di-highlight dengan HIGHLIGHT; bar terendah LOW."""
    colors = [BASE] * len(values)
    colors[int(np.argmax(values))] = HIGHLIGHT
    colors[int(np.argmin(values))] = LOW
    return colors
 
with c3:
    if not day_f.empty:
        season_avg = (day_f.groupby("season", observed=True)["cnt"]
                      .mean().reset_index()
                      .sort_values("cnt", ascending=False))
 
        colors = bar_colors_highlight(season_avg["cnt"].values)
 
        fig, ax = plt.subplots(figsize=(6, 4))
        bars = ax.bar(season_avg["season"], season_avg["cnt"],
                      color=colors, edgecolor="white", width=0.6)
        ax.bar_label(bars, fmt="%.0f", padding=3, fontsize=10, fontweight="bold")
        ax.set_xlabel("Musim", labelpad=6)
        ax.set_ylabel("Rata-rata Disewa per Hari")
        ax.set_title("Rata-rata Penyewaan per Musim", fontsize=11, fontweight="bold")
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}"))
        ax.grid(axis="y", alpha=0.25, linestyle="--")
        ax.spines[["top", "right"]].set_visible(False)
        ...
        # Legenda ringkas
        legend_els = [
            mpatches.Patch(color=HIGHLIGHT, label="Tertinggi"),
            mpatches.Patch(color=BASE,      label="Lainnya"),
            mpatches.Patch(color=LOW,       label="Terendah"),
        ]
        ax.legend(handles=legend_els, fontsize=8, loc="upper right", framealpha=0.7)
        fig.tight_layout()
        st.pyplot(fig); plt.close(fig)
        ...
        insight("📌", "Poin Penting",
                f"<b>{best_season}</b> adalah waktu favorit untuk bersepeda "
                f"(rata-rata <b>{best_val:,} kali disewa per hari</b>). "
                f"Cuaca yang sejuk bikin orang lebih suka bersepeda.")
 
with c4:
    if not day_f.empty:
        weather_avg = (day_f.groupby("weathersit", observed=True)["cnt"]
                       .mean().reset_index()
                       .sort_values("cnt", ascending=False))
 
        colors = bar_colors_highlight(weather_avg["cnt"].values)
 
        fig, ax = plt.subplots(figsize=(6, 4))
        bars = ax.barh(weather_avg["weathersit"], weather_avg["cnt"],
                       color=colors, edgecolor="white", height=0.55)
        ax.bar_label(bars, fmt="%.0f", padding=3, fontsize=10, fontweight="bold")
        ax.set_xlabel("Rata-rata Disewa per Hari")
        ax.set_title("Rata-rata Penyewaan per Kondisi Cuaca", fontsize=11, fontweight="bold")
        ...
        ax.legend(handles=legend_els, fontsize=8, loc="lower right", framealpha=0.7)
        fig.tight_layout()
        st.pyplot(fig); plt.close(fig)
        ...
        insight("📌", "Poin Penting",
                ...
                f"Begitu cuaca memburuk seperti <b>{worst_w}</b>, jumlah pesepeda langsung anjlok.",
                color="yellow")
...
if not hour_c.empty:
    ...
    with c5:
        # Highlight tertinggi & terendah, sisanya BASE
        vals   = agg["rata_rata"].values
        colors = bar_colors_highlight(vals)
        ...
        legend_els = [
            mpatches.Patch(color=HIGHLIGHT, label="Jam Tersibuk"),
            mpatches.Patch(color=BASE,      label="Lainnya"),
            mpatches.Patch(color=LOW,       label="Jam Tersepi"),
        ]
    ...
    with c6:
        # Stacked bar komposisi — warna konsisten: oranye=casual, biru=registered
        fig, ax = plt.subplots(figsize=(7, 4.5))
        x = list(range(len(cluster_order)))
        ax.barh(x, pct_casual, color=COL_CAS, edgecolor="white", height=0.65, label="Penyewa Biasa")
        ax.barh(x, pct_reg, left=pct_casual, color=COL_REG, edgecolor="white", height=0.65, label="Member Terdaftar")
        for i, (pc, pr) in enumerate(zip(pct_casual, pct_reg)):
            if pc > 5: ax.text(pc / 2,      i, f"{pc:.0f}%", va="center", ha="center",
                               fontsize=9, fontweight="bold", color="white")
            if pr > 5: ax.text(pc + pr / 2, i, f"{pr:.0f}%", va="center", ha="center",
                               fontsize=9, fontweight="bold", color="white")
        ax.set_yticks(x)
    ...
    with ic1:
        insight("🔵", "Jam Tersibuk",
                f"Jam <b>{peak_seg}</b> sangat padat. Biasanya ini adalah orang-orang kantoran "
                f"yang pakai sepeda untuk berangkat atau pulang kerja.", color="orange")
    with ic2:
        insight("⬜", "Jam Paling Sepi",
                f"Di <b>{quiet_seg}</b> jalanan sangat sepi penyewa. Tentu saja, "
                f"karena mayoritas orang masih tidur/beristirahat.", color="green")
    with ic3:
        insight("🟠", "Favorit Jalan-Jalan",
                f"Jam <b>{most_casual_seg}</b> paling disukai oleh penyewa biasa. "
                f"Biasanya mereka menyewa untuk jalan-jalan santai.")
...
with c7:
    ...
    if not temp_data.empty:
        ...
        temp_data["temp_bin"] = pd.cut(temp_data["temp_c"], bins=temp_bins,
                                       labels=temp_labels, right=False)
        bin_avg = (temp_data.groupby("temp_bin", observed=True)["cnt"]
                   .mean().reindex(temp_labels).fillna(0))
 
        colors = bar_colors_highlight(bin_avg.values)
 
        fig, ax = plt.subplots(figsize=(6, 4.5))
        bars = ax.bar(range(len(temp_labels)), bin_avg.values,
                      color=colors, edgecolor="white", width=0.62)
        ax.bar_label(bars, fmt="%.0f", padding=3, fontsize=10, fontweight="bold")
        ...
        legend_els = [
            mpatches.Patch(color=HIGHLIGHT, label="Suhu Favorit"),
            mpatches.Patch(color=BASE,      label="Lainnya"),
            mpatches.Patch(color=LOW,       label="Paling Sepi"),
        ]
        ax.legend(handles=legend_els, fontsize=8, loc="upper left", framealpha=0.7)
        fig.tight_layout()
        st.pyplot(fig); plt.close(fig)
        ...
        insight("📌", "Poin Penting",
                f"Suhu ideal untuk bersepeda ada di kisaran <b>{peak_bin}</b> "
                f"(rata-rata <b>{peak_val:,} sewaan/hari</b>). "
...
with c8:
    if not day_f.empty:
        ...
        x      = np.arange(len(wday.index))
        width  = 0.35
        bars_c = ax.bar(x - width / 2, wday["casual"],     width,
                        color=COL_CAS, edgecolor="white", label="Penyewa Biasa")
        bars_r = ax.bar(x + width / 2, wday["registered"], width,
                        color=COL_REG, edgecolor="white", label="Member Terdaftar")
 
        for container in [bars_c, bars_r]:
            ax.bar_label(container, fmt="%.0f", padding=3, fontsize=10, fontweight="bold")
 
        ax.set_xticks(x)
        ax.set_xticklabels(wday.index, fontsize=10)
        ax.set_ylabel("Rata-rata Disewa per Hari")
        ax.set_title("Beda Pola: Hari Kerja vs Hari Libur", fontsize=11, fontweight="bold")
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}"))
        ax.legend(["Penyewa Biasa", "Member Terdaftar"], fontsize=9, framealpha=0.7)
        ax.grid(axis="y", alpha=0.25, linestyle="--")
        ax.spines[["top", "right"]].set_visible(False)
        fig.tight_layout()
        st.pyplot(fig); plt.close(fig)
        ...
        insight("📌", "Poin Penting",
                f"Di <b>Hari Kerja</b>, sepeda penuh dipakai oleh para member "
                f"(<b>{reg_kerja:,}/hari</b>) buat pergi bekerja. "
                f"Tapi pas <b>Hari Libur</b>, penyewa biasa yang cuma jalan-jalan santai "
                f"jumlahnya naik drastis (jadi <b>{cas_libur:,} orang/hari</b>).",
                color="orange")
